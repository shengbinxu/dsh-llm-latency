...
import { emptyStore, recordSample } from './metrics.js';
...
function migrateV1(parsed, opts) {
    ...
    for (const agg of Object.values(keys)) {
        ...
        for (const raw of recent) {
            ...
            if (sample !== null)
                recordSample(store, sample, opts);
        }
    }
    return store;
}
function migrateV1Sample(raw) {
    if (raw === null || typeof raw !== 'object')
        return null;
    const r = raw;
    if (typeof r.ts !== 'number' || typeof r.vendor !== 'string')
        return null;
    const kind = r.errorKind === 'aborted' ? 'aborted' : r.errorKind === 'error' ? 'other' : null;
    return {
        ts: r.ts,
        vendor: r.vendor,
 ...
 */
export function loadStore(path, opts = DEFAULT_RECORD_OPTIONS) {
    try {
        if (!existsSync(path))
