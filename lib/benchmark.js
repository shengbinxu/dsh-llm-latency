...
export const benchmarkOptions = new Set();
function buildOptions(base, target, nonce, opts) {
    const messages = [...base.messages, { role: 'user', content: `\n\n[llm-latency-bench ${nonce}]` }];
...
async function measureOne(llm, base, target, round, opts) {
...
export async function runBenchmark(llm, base, targets, opts, onSample) {
    const byTarget = new Map();
    const keyOf = (t) => `${t.provider}|${t.model}`;
    ...
    for (const target of targets) {
        ...
        const ttft = ok.map((s) => s.ttftMs).filter((v) => v !== null).sort((a, b) => a - b);
        const e2e = ok.map((s) => s.e2eMs).filter((v) => v !== null).sort((a, b) => a - b);
        let outputTokens = 0;
        let decodeMs = 0;
        let cacheRead = 0;
        let inputTokens = 0;
...
export function smallPercentile(values, p) {
