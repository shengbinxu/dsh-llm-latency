...
import { summarizeSession, type SessionSummary } from './metrics.js'
...
export function compareSessions(store: StatsStore, ids: readonly string[], aliases: ModelAliases): SessionCompareResult {
  const rows: SessionCompareRow[] = []
  for (const id of ids) {
