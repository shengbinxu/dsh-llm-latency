...
import { summarizeStore, type StatsStore } from './metrics.js'
...
export function formatSummaryTable(store: StatsStore): string {
  ...
  for (const r of rows) {
    lines.push(
      `| ${r.vendor} · ${r.model} | ${r.okCount}/${r.count} | ${ms(r.ttftP50)} | ${ms(r.ttftP95)} | ${ms(r.e2eP50)} | ${tps(r.tokensPerSecond)} | ${pct(r.cacheHitPct)} | ${r.spikes} |`,
    )
  }
  return lines.join('\n')
}
...
export function formatBenchmarkTable(results: BenchmarkTargetResult[]): string {
  if (results.length === 0) return '无对拍结果。'
  const lines = [
  ...
  for (const r of results) {
    lines.push(
      `| ${r.vendor} · ${r.model} | ${r.okCount}/${r.rounds} | ${ms(r.ttftP50)} | ${ms(r.ttftP95)} | ${ms(r.e2eP50)} | ${tps(r.tokensPerSecond)} | ${pct(r.cacheHitPct)} |`,
    )
  }
  return lines.join('\n')
}
