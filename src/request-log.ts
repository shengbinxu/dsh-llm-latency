...
export function createRequestLogStore(path: string, limit: number, retentionDays: number): RequestLogStore {
  return {
...
export function loadRequestLog(path: string, limit: number, retentionDays: number): Sample[] {
  try {
...
export function appendRequestLog(store: RequestLogStore, sample: Sample): void {
  store.records.push(sample)
  if (store.records.length > store.limit) store.records.splice(0, store.records.length - store.limit)
  try {
...
export function compactRequestLog(store: RequestLogStore): void {
  try {
...
export function queryRequestLog(
  ...
  limit: number,
  offset: number,
): { records: Sample[]; total: number } {
  const q = filter.q?.trim().toLowerCase()
  const filtered = store.records.filter((s) => {
    if (filter.vendor !== undefined && s.vendor !== filter.vendor) return false
    if (filter.model !== undefined && s.model !== filter.model) return false
    if (filter.from !== undefined && s.ts < filter.from) return false
    if (filter.to !== undefined && s.ts > filter.to) return false
